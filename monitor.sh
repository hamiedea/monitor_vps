...
warn() {
    echo -e "\033[33m[PERINGATAN]\033[0m $1"
}
...
clear
echo "==============================================="
echo "   Installer Otomatis Bot Monitor VPS"
echo "==============================================="
echo

# 1. PERSIAPAN SISTEM
info "Memperbarui daftar paket dan menginstal dependensi (python3, pip, screen)..."
sudo apt-get update > /dev/null 2>&1
sudo apt-get install -y python3 python3-pip screen > /dev/null 2>&1
info "Dependensi berhasil diinstal."
echo

# 2. PERSIAPAN DIREKTORI APLIKASI
info "Mempersiapkan direktori aplikasi di ~/${APP_DIR}..."
# Kembali ke home directory untuk memastikan
cd ~
if [ -d "$APP_DIR" ]; then
    warn "Direktori ~/${APP_DIR} sudah ada. File yang ada mungkin akan ditimpa."
else
    mkdir -p "$APP_DIR"
fi
cd "$APP_DIR"
info "Berada di direktori `pwd`"
echo

# 3. MEMBUAT FILE BOT.PY
info "Membuat file program bot.py..."
cat > "$BOT_FILE" << 'EOF'
#!/usr/bin/env python3
import asyncio
import json
...
if __name__ == '__main__':
    main()
EOF
info "File bot.py berhasil dibuat."
echo

# 4. INSTAL LIBRARY PYTHON
info "Menginstal library python-telegram-bot dengan job-queue..."
pip3 install "python-telegram-bot[job-queue]" > /dev/null 2>&1
info "Library Python berhasil diinstal."
echo

# 5. KONFIGURASI INTERAKTIF
info "Memulai konfigurasi interaktif..."
BOT_TOKEN=""
while [ -z "$BOT_TOKEN" ]; do
    read -p "Masukkan BOT_TOKEN Anda: " BOT_TOKEN
    if [ -z "$BOT_TOKEN" ]; then
        warn "BOT_TOKEN tidak boleh kosong. Silakan masukkan lagi."
    fi
done

read -p "Masukkan interval monitor (detik) [Default: 120]: " MONITOR_INTERVAL
MONITOR_INTERVAL=${MONITOR_INTERVAL:-120}

info "Membuat file config.json..."
cat > "$CONFIG_FILE" << EOF
{
  "BOT_TOKEN": "$BOT_TOKEN",
...
# 6. MENJALANKAN BOT
info "Mengecek apakah sesi screen '$SCREEN_SESSION' sudah berjalan..."
if screen -list | grep -q "$SCREEN_SESSION"; then
    warn "Sesi screen '$SCREEN_SESSION' sudah ada. Menghentikan sesi lama..."
    screen -X -S "$SCREEN_SESSION" quit
fi

info "Menjalankan bot dalam sesi screen '$SCREEN_SESSION'..."
screen -dmS "$SCREEN_SESSION" python3 "$BOT_FILE"

# --- SELESAI ---
echo
echo "======================================================"
info "SELURUH PROSES INSTALASI DAN KONFIGURASI SELESAI!"
echo "======================================================"
echo
echo "Bot Anda sekarang berjalan di latar belakang."
echo "Untuk melihat log bot, jalankan perintah:"
echo -e "  \033[36mscreen -r $SCREEN_SESSION\033[0m"
echo
echo "Untuk keluar dari tampilan log (tanpa mematikan bot), tekan CTRL+A lalu D."
echo
echo "Untuk mematikan bot secara permanen, jalankan perintah:"
echo -e "  \033[36mscreen -X -S $SCREEN_SESSION quit\033[0m"
echo
